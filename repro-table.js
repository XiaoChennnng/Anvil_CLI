...
const ok = '| 事件 | 要点 |\n|------|------|\n| 中美关税战升级 | 加征 34% |\n| 俄乌冲突加剧 | 普京提议 |\n';
const out1 = r1.write(ok);
console.log('OUT:', JSON.stringify(stripAnsi(out1)));
console.log('HAS TABLE:', out1.includes('─'));

console.log('\n=== Test 2: 整段挤一行（无 \\n）===');
const r2 = new MarkdownRenderer(80);
const bad = '| 事件 | 要点 | |------|------| | 中美关税战升级 | 加征 34% | | 俄乌冲突加剧 | 普京提议 |';
const out2 = r2.write(bad);
console.log('OUT:', JSON.stringify(stripAnsi(out2)));
console.log('HAS TABLE:', out2.includes('─'));

console.log('\n=== Test 3: 用户截图里的"国内大事"格式 ===');
const r3 = new MarkdownRenderer(80);
const user = '### 🇨🇳国内大事|事件 |要点 | |------|------| | 神舟二十号发射成功 |4月24日 | | 国产飞机C909老挝首航 |4月12日 |';
const out3 = r3.write(user);
console.log('OUT:', JSON.stringify(stripAnsi(out3)));
console.log('HAS TABLE:', out3.includes('─'));